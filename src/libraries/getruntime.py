...
def lastmonthtotaltime(mydb, system):
    ...
    if datetime.date.today().day < 15:
        ...
        if datetime.date.today().month < 3:
            billingstart = datetime.date.today().replace(year=(datetime.date.today().year - 1))
            billingend = datetime.date.today().replace(year=(datetime.date.today().year - 1))

            if datetime.date.today().month == 1:
                billingend = billingstart.replace(month=12)
                billingstart = billingend.replace(month=11)
            else:
                billingend = billingstart.replace(month=11)
                billingstart = billingend.replace(month=10)
        else:
                billingendmonth = datetime.date.today().month - 1
                billingstartmonth = datetime.date.today().month - 2
                billingend = datetime.date.today().replace(month=billingendmonth)
        ...
        # if its after the 15th, set the billing end to this month, day = 15
        billingend = datetime.date.today().replace(month=datetime.date.today().month)
        billingend = billingend.replace(day=15)

        billingstart = datetime.datetime.today()
        # if its jan then we need to look at decemeber for the starting period
        if datetime.date.today().month == 1:
            billingstart = datetime.date.today().replace(year=datetime.date.today().year - 1)
            billingstart = billingstart.replace(month=12)
            billingstart = billingstart.replace(day=15)

        # otherwise use current monnth - 1
        else:
            billingstart = datetime.date.today().replace(month=(datetime.date.today().month - 1))
            billingstart = billingstart.replace(day=15)

    totaldays = (billingend - billingstart).days
    ...
    while billingend > billingstart:
        ...
        billingstart = billingstart + datetime.timedelta(days=1)
...
def monthltime (mydb, system):
    #if its not the 15th yet, look at -1 months 15th
    if datetime.date.today().day < 15:
        if datetime.date.today().month == 1:
            ...
            billingstartmonth = datetime.date.today().month - 1
            billingstart = datetime.date.today().replace(month=billingstartmonth)
            billingstart = billingstart.replace(day=15)

        totaldays = (datetime.date.today() - billingstart).days

    else:
        billingstart = datetime.date.today().replace(day=15)
        totaldays = (datetime.date.today() - billingstart).days

    history = gethistory(totaldays, system, mydb)
    totaltime = 0
    ...
    multiplier = 10 ** 2
    return int(totaltime * multiplier) / multiplier, totaldays
...
def gethistory(range, system, mydb):
    start = 0
    history = []
    onhistory = []
    while start < range:
        ddate = str(datetime.date.today() - datetime.timedelta(days=start))
        hisdf = getdata(ddate, system, mydb)
...
if __name__ == "__main__":
    ...
    if sys.argv[-1] == "--ac":
        print(gethistory(7, "ac", mydb))

    elif sys.argv[-1] == "--heat":
        print(gethistory(7, "heat", mydb))
