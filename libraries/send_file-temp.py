...
def my_publish(msg):
    try:
        client.publish(SUBTOPIC, my_json(msg), qos=0)
        #if msg["end"] is False:
        #    print(
    ...
    with open(myfile, 'rb') as f:
        while True:
            ...
            if chunk:
                ...
                #loggerdo.log.debug("END transfer file:", myfile)
                my_publish(payload)
                break
    time.sleep(1)
    client.disconnect()
...
def my_event(top, msg):
    ...
    try:
        ...
        #loggerdo.log.debug("ERR: in json", e)
        #my_exit(3)
        return
...
def on_connect(client, userdata, flags, rc):
    ...
    #loggerdo.log.debug("subscribe to:", SUBTOPIC)
...
def main(ip, topic, myfile):
    global HOST, SUBTOPIC
    HOST = ip
    SUBTOPIC = topic +"/graph"
